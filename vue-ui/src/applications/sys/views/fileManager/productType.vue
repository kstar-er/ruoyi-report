  ...
  <div>
    ...
    <div class="extend-handle">
      <div class="extend-handle-left">
        <xButton
          v-authorityHandle="'wms:category:add'"
          @click="addRow"
        >
          新增
        </xButton>
...
import { DataSource, loading } from './utils/productType'
...
const deleteRow = async (rowData) => {
  proxy.$alert(`是否确认删除此类别，只会删除该项，不会删除子类别，确认请点击继续！`, '提示', {
    type: 'error',
    ...
    callback: (action) => {
      if (action === 'cancel') return
        ...
        dataSource.value.deleteTypeWithOutSon(rowData.id).then((res) => {
          if (res.code === 200){
...
const deleteRows = async (rowData) => {
  proxy.$alert(`是否确认删除此类别，注意：如果该类别有子类别，将会一并删除，确认请点击继续！`, '提示', {
    type: 'error',
    showCancelButton: true,
    cancelButtonText: '再想想',
    confirmButtonText: '继续',
    ...
    callback: (action) => {
      if (action === 'cancel') return
        ...
        dataSource.value.deleteTypeWithSon(rowData.id).then((res) => {
          if (res.code === 200){
            proxy.$message.success('删除成功')
            dataSource.value.initData(this, proxy.$refs.table)
          }
...
const customizeEndHandle = (rowData, rowIndex) => {
  return [h('div', { class: 'table-handel-div' }, [withDirectives(h(
    ElButton,
    { class: 'hover-animation', onClick: () => { goCompile(rowData) }, title: '更新类别', style: 'padding:0 10px;margin:0;background:transparent;width:80px;color:#0c81b1;margin-right:10px;font-size:12px' },
    { default: () => "更新类别" }), [[authority, 'wms:category:edit']]
  ), withDirectives(h(
    ElButton,
    { class: 'hover-animation', onClick: () => { addRow(rowData) }, title: '添加子类', style: 'padding:0 10px;margin:0;background:transparent;width:80px;color:#0c81b1;margin-right:10px;font-size:12px' },
    { default: () => "添加子类" }), [[authority, 'wms:category:edit']]
  ), withDirectives(h(
    ElButton,
    { class: 'hover-animation', onClick: () => { deleteRow(rowData) }, title: '删除类别', style: 'padding:0 10px;margin:0;background:transparent;width:80px;color:#e81123;margin-right:10px;font-size:12px' },
    { default: () => "删除类别" }), [[authority, 'wms:category:remove']]
  ), withDirectives(h(
    ElButton,
    { class: 'hover-animation', onClick: () => { deleteRows(rowData) }, title: '删除此组', style: 'padding:0 10px;margin:0;background:transparent;width:80px;color:#e81123;font-size:12px' },
    { default: () => "删除此组" }), [[authority, 'wms:category:remove']]
  )])]
}
...
const editControlCommon = reactive({
  ...
  inputDone: async (val) => {
    ...
    if (params.id){
      let res = await dataSource.value.updateType(params)
      if (res.code === 200){
      ...
      let res = await dataSource.value.addType(params)
      if (res.code === 200){
