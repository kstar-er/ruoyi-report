    ...
    <div class="header">
      <el-page-header @back="goBack">
        <template #content>
          ...
          <el-button
            :disabled="formData.processStatus&&formData.processStatus==='inventory_doc'"
            ...
            >
              <template #default="scope">
                <el-input-number
                  ...
                  :disabled="formData.processStatus!=='inventory_ing'"
                  :min="0"
...
const active = ref(`${route.query.id ? route.query.inventorySheetOrderCode + ' 调拨单明细' : '新增调拨单'} `)
...
const myId = ref(-1)
const inventorySheetOrderCode = ref(-1)
const initData = async () => {
  ...
  if (!listDataSource.value){
    listDataSource.value = new DataSource({
      selectUri: '/wms/sheetDetail/list',
      listMethod: 'GET',
      tableHeader: [
        {
          title: "产品代码",
...
const getDetail = async (val) => {
  ...
  inventorySheetOrderCode.value = route.query.inventorySheetOrderCode ? route.query.inventorySheetOrderCode : val.inventorySheetOrderCode
  dataSource.value.getSheet(myId.value).then(async res => {
    if (res.code === 200){
      formData.value = { ...res.data }
      editControlCommon.changeData = !editControlCommon.changeData
      listDataSource.value.searchData = { inventorySheetOrderCode: formData.value.inventorySheetOrderCode }
  ...
  inputDone: async (val, type) => {
    ...
    if (!val){
      ...
      if (params.id){
        ...
        let res = await dataSource.value.addSheet(params)
        if (res.code === 200){
...
const openSelectDialog = async (type) => {
  ...
  case 'product': {
    ...
    selectTableData.submit = () => {
      listDataSource.value.tableData.unshift(...selectTableData.dataSource.selections)

      let temp = listDataSource.value.tableData.filter((item, index) => index === listDataSource.value.tableData.findIndex(i => {
        i.bookQuantity = i.sumNum
        i.firmOfferQuantity = 0
        i.profitAndLossQuantity = 0
        return i.productCode === item.productCode
      }))
      listDataSource.value.tableData.length = 0
      listDataSource.value.tableData.push(...temp)
  ...
  proxy.$alert(`是否确认递交该盘点单？`, '提示', {
    ...
    callback: (action) => {
      if (action === 'cancel') return
        ...
        dataSource.value.nextProcess(formData.value.id).then((res) => {
          if (res.code === 200){
