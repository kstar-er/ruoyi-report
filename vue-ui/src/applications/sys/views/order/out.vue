...
const initDataSource = async () => {
  ...
  dataSource.value = new DataSource({
    modules: 'out',
    selectUri: '/wms/OutWarehouseOrderMain/list',
    listMethod: 'GET',
    pageSize: 20
  })
  dataSource.value.initTableHeader()
  dataSource.value.searchData.processStatusList = 'out_wait_out,out_out_ing'
...
const goCompile = async (rowData) => {
  let type = rowData.type === '调拨出库' ? 'allocation' : 'product'
  router.push(`/order/outDetail?type=${type}&id=${rowData.id}&outWarehouseCode=${rowData.outWarehouseCode}`)
}

const addRow = async (rowData) => {
  router.push('/order/outDetail?type=allocation')
}

const addRowProduct = async (rowData) => {
  router.push('/order/outDetail?type=product')
}
...
const deleteRow = async (rowData) => {
  if (rowData.processStatus === 'out_wait_out' || rowData.processStatus === 'out_create'){
    ...
    proxy.$alert(`该流程状态不允许删除，如有需要，请尝试使用作废功能`, '提示', {
      type: 'error',
      showCancelButton: true,
      cancelButtonText: '确认作废',
